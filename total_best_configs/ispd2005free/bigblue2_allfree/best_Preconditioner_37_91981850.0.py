    ...
    def __call__(self, grad, density_weight, update_mask=None, fix_nodes_mask=None):
        ...
        with torch.no_grad():
            ...
            if density_weight.size(0) == 1:
                # Step 1: Base hierarchical estimation using node areas
                base_factor = torch.mean(self.data_collections.node_areas) * self.alpha

                # Step 2: Multigrid refinement with cell variance and pin statistics
                pin_mean = torch.mean(self.data_collections.pin_offset_x + self.data_collections.pin_offset_y).to(base_factor.device)
                node_variance = torch.var(self.data_collections.node_size_x + self.data_collections.node_size_y).to(base_factor.device)
                refined_factor = base_factor + (pin_mean + node_variance) * self.alpha
                refined_factor = refined_factor.clamp(min=0.1).to(base_factor.device)
                ...
                wirelength_approx = (refined_factor + net_halflength_avg + pin_variance * self.alpha).clamp(min=0.2)

                # Step 4: Advanced density estimation using Gaussian Mixture Model approach
                gmm_modelling = torch.var(self.data_collections.num_pins_in_nodes.float()).to(wirelength_approx.device)
                density_variation = wirelength_approx * (1 + gmm_modelling).clamp(max=1.8).to(wirelength_approx.device)

                # Step 5: Adaptive region-specific preconditioning
                historical_mean = torch.mean(wirelength_approx).to(density_variation.device)
                adaptive_noise = historical_mean * torch.randn_like(density_variation).to(density_variation.device)
                region_based_factor = wirelength_approx * (1 + adaptive_noise * self.alpha * 0.6).clamp(min=0.1, max=0.6).to(historical_mean.device)
