...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Retrieve parameters for improved initialization
        xl, xh = placedb.xl, placedb.xh
        yl, yh = placedb.yl, placedb.yh
        row_height, site_width = placedb.row_height, placedb.site_width

        movable_density = placedb.total_movable_node_area / placedb.total_space_area
        macro_density = placedb.total_movable_macro_area / placedb.total_space_area
        cell_density = placedb.total_movable_cell_area / placedb.total_space_area

        # Use standard deviations based on different densities
        macro_scale_x = (xh - xl) * 0.05 * macro_density
        macro_scale_y = (yh - yl) * 0.05 * macro_density
        cell_scale_x = (xh - xl) * 0.1 * cell_density
        cell_scale_y = (yh - yl) * 0.1 * cell_density

        # Initialize movable cells and macros
        for i in range(placedb.num_movable_nodes):
            if placedb.node_size_x[i] * placedb.node_size_y[i] >= row_height * site_width * 100:
                # Likely a macro
                loc_x = placedb.node_x[i] if xl < placedb.node_x[i] < xh else (xl + xh) / 2
                loc_y = placedb.node_y[i] if yl < placedb.node_y[i] < yh else (yl + yh) / 2
                self.init_pos[i] = np.random.normal(loc=loc_x, scale=macro_scale_x if macro_scale_x > 0 else 1)
                self.init_pos[placedb.num_nodes + i] = np.random.normal(loc=loc_y, scale=macro_scale_y if macro_scale_y > 0 else 1)
            else:
                # Likely a standard cell
                loc_x = placedb.node_x[i] if xl < placedb.node_x[i] < xh else (xl + xh) / 2
                loc_y = placedb.node_y[i] if yl < placedb.node_y[i] < yh else (yl + yh) / 2
                self.init_pos[i] = np.random.normal(loc=loc_x, scale=cell_scale_x if cell_scale_x > 0 else 1)
                self.init_pos[placedb.num_nodes + i] = np.random.normal(loc=loc_y, scale=cell_scale_y if cell_scale_y > 0 else 1)

        # Initial positions for fixed cells remain unchanged
        self.init_pos[placedb.num_movable_nodes:placedb.num_physical_nodes] = placedb.node_x[placedb.num_movable_nodes:placedb.num_physical_nodes]
        self.init_pos[placedb.num_nodes + placedb.num_movable_nodes:placedb.num_nodes + placedb.num_physical_nodes] = placedb.node_y[placedb.num_movable_nodes:placedb.num_physical_nodes]

        logging.info("Density-aware and connectivity-aware initialization completed in {:.3f} seconds.".format(time.time() - tt))
