    ...
    def __call__(self, grad, density_weight, update_mask=None, fix_nodes_mask=None):
        ...
        with torch.no_grad():
            ...
            if density_weight.size(0) == 1:
                # Step 1: Compute initial factors based on node areas
                hierarchical_factor = torch.mean(self.data_collections.node_areas) * self.alpha
                
                # Step 2: Multigrid refinement using cell variance and pin statistics
                pin_mean = torch.mean(self.data_collections.pin_offset_x + self.data_collections.pin_offset_y).to(hierarchical_factor.device)
                ...
                refined_factor = hierarchical_factor + (pin_mean + node_variance) * self.alpha
                refined_factor = refined_factor.clamp(min=0.1).to(hierarchical_factor.device)
                
                # Step 3: Estimate wirelength with geometric and pin-based factors
                net_halflength_avg = torch.mean(self.data_collections.node_size_x + self.data_collections.node_size_y).to(refined_factor.device)
                pin_variance = torch.var(self.data_collections.num_pins_in_nodes.float()).to(refined_factor.device)
                wirelength_estimate = (refined_factor + net_halflength_avg + pin_variance * self.alpha).clamp(min=0.2)
                
                # Step 4: Use an adaptive density estimation with a mixture model for robustness
                gmm_modelling = torch.var(self.data_collections.num_pins_in_nodes.float()).to(wirelength_estimate.device)
                adaptive_density = wirelength_estimate * (1 + gmm_modelling).clamp(max=2.0).to(wirelength_estimate.device)
                
                # Step 5: Region-based adaptive preconditioning
                historical_mean = torch.mean(adaptive_density).to(adaptive_density.device)
                adaptive_noise = historical_mean * torch.randn_like(adaptive_density).to(adaptive_density.device)
                region_preconditioner = adaptive_density * (1 + self.alpha * adaptive_noise).clamp(min=0.1, max=0.7).to(historical_mean.device)
                
                # Compute the final preconditioner integrating pin weight and density factors
                sum_pin_weights = self.op_collections.pws_op(self.data_collections.net_weights.to(region_preconditioner.device))
                precond = (sum_pin_weights + region_preconditioner * self.alpha * density_weight * self.data_collections.node_areas.to(region_preconditioner.device))
            ###pre-condition-end###
            else:
