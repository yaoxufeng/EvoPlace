...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Calculate chip center
        center_x = (placedb.xl + placedb.xh) / 2
        center_y = (placedb.yl + placedb.yh) / 2

        # Calculate the density of movable cells
        density_factor = placedb.total_movable_cell_area / placedb.total_space_area

        # Improved x position initialization with consideration of density and net weights
        x_std_deviation = density_factor * (placedb.xh - placedb.xl) * 0.05
        self.init_pos[0:placedb.num_physical_nodes] = placedb.node_x
        if params.global_place_flag:
            logging.info("Applying density and net-weighted x-position initialization.")
            self.init_pos[0:placedb.num_movable_nodes] = np.random.normal(
                loc=center_x + np.mean(placedb.net_weights) * 0.1,
                scale=x_std_deviation,
                size=placedb.num_movable_nodes
            )

        # Improved y position initialization
        y_std_deviation = density_factor * (placedb.yh - placedb.yl) * 0.05
        self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_physical_nodes] = placedb.node_y
        if params.global_place_flag:
            logging.info("Applying density and net-weighted y-position initialization.")
            self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_movable_nodes] = np.random.normal(
                loc=center_y + np.mean(placedb.net_weights) * 0.1,
                scale=y_std_deviation,
                size=placedb.num_movable_nodes
            )
###move-layout-end###
