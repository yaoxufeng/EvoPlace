...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Density-related computations
        density_ratio = placedb.total_movable_node_area / placedb.total_space_area
        spread_factor_x = 1.0 - density_ratio
        spread_factor_y = 1.0 - density_ratio
        
        if params.global_place_flag:
            logging.info("Strategically place cells using density and area information for improved initial placement")
            
            # Initialize movable node positions with improved spreading
            noise_scale_x = (placedb.xh - placedb.xl) * 0.005 * spread_factor_x
            noise_scale_y = (placedb.yh - placedb.yl) * 0.005 * spread_factor_y
            
            # Noise for better distribution and avoiding congestion
            noise_x = np.random.normal(loc=center_x, scale=noise_scale_x, size=placedb.num_movable_nodes)
            noise_y = np.random.normal(loc=center_y, scale=noise_scale_y, size=placedb.num_movable_nodes)
            
            # Initialize positions considering both density and spread factor
            self.init_pos[0:placedb.num_movable_nodes] = placedb.node_x[0:placedb.num_movable_nodes] * spread_factor_x + noise_x
            self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_movable_nodes] = placedb.node_y[0:placedb.num_movable_nodes] * spread_factor_y + noise_y
        
        else:
            # Standard initial positions if global placement flag not set
        ...
        # Initialize filler nodes if they exist, ensuring they don't overlap and are well-spread
        if placedb.num_filler_nodes:
            filler_x = np.random.uniform(low=placedb.xl, high=placedb.xh - placedb.node_size_x[-1], size=placedb.num_filler_nodes)
            filler_y = np.random.uniform(low=placedb.yl, high=placedb.yh - placedb.node_size_y[-1], size=placedb.num_filler_nodes)
            
            self.init_pos[placedb.num_physical_nodes:placedb.num_nodes] = filler_x
            self.init_pos[placedb.num_nodes + placedb.num_physical_nodes:placedb.num_nodes * 2] = filler_y
