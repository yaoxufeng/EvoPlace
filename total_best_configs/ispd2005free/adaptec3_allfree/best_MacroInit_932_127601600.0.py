    ...
    def __init__(self, pos, params, placedb, device):
        ...
        with torch.no_grad():
            ...
            if len(placedb.regions) > 0:
                ...
                self.virtual_macro_fence_region = [torch.from_numpy(region).to(device) for region in
                                                   placedb.virtual_macro_fence_region]
                ## this is for overflow op
                self.total_movable_node_area_fence_region = torch.from_numpy(
                    placedb.total_movable_node_area_fence_region).to(device)
                ## this is for gamma update
                self.num_movable_nodes_fence_region = torch.from_numpy(placedb.num_movable_nodes_fence_region).to(
                    device)
...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Provide the center coordinates of the layout
        center_x = (placedb.xl + placedb.xh) / 2
        center_y = (placedb.yl + placedb.yh) / 2
        
        # Calculate statistical information for improved initialization
        avg_cell_area = placedb.total_movable_cell_area / placedb.num_movable_nodes
        avg_macro_area = placedb.total_movable_macro_area / placedb.num_nodes if placedb.total_movable_macro_area > 0 else 0
        avg_net_weight = np.mean(placedb.net_weights)
        
        # Initial positions of standard cells
        self.init_pos[:placedb.num_physical_nodes] = placedb.node_x
        ...
        if params.global_place_flag:
            if params.random_center_init_flag:
                # Distribute macros around the layout
                movable_x_positions = []
                movable_y_positions = []
                
                for i in range(placedb.num_movable_nodes):
                    macro_width = placedb.node_size_x[i]
                    macro_height = placedb.node_size_y[i]
                    
                    # Define initial random positioning with slight perturbation in center
                    new_x = np.random.normal(loc=center_x, scale=(placedb.xh - placedb.xl) * 0.1)
                    new_y = np.random.normal(loc=center_y, scale=(placedb.yh - placedb.yl) * 0.1)
                    
                    # Ensure the positions are within the chip boundaries
                    new_x = max(min(new_x, placedb.xh - macro_width / 2), placedb.xl + macro_width / 2)
                    new_y = max(min(new_y, placedb.yh - macro_height / 2), placedb.yl + macro_height / 2)
                    
                    # Append positions
                    movable_x_positions.append(new_x)
                    movable_y_positions.append(new_y)
                
                self.init_pos[:placedb.num_movable_nodes] = np.array(movable_x_positions)
                self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_movable_nodes] = np.array(movable_y_positions)
            else:
                # Enhanced Grid-based placement to avoid density bottlenecks and consider pin connectivity
                cell_width = np.mean(placedb.node_size_x)
                cell_height = np.mean(placedb.node_size_y)
                grid_spacing_x = placedb.bin_size_x
                grid_spacing_y = placedb.bin_size_y
                
                grid_positions = [(placedb.xl + i * grid_spacing_x, placedb.yl + j * grid_spacing_y)
                                  for i in range(int((placedb.xh - placedb.xl) / grid_spacing_x))
                                  for j in range(int((placedb.yh - placedb.yl) / grid_spacing_y))]
                
                np.random.shuffle(grid_positions)  # Shuffle to avoid clustering in any specific region
                
                for i in range(placedb.num_movable_nodes):
                    grid_x, grid_y = grid_positions[i % len(grid_positions)]
                    
                    # Add some randomness
                    grid_x += np.random.uniform(-cell_width / 2, cell_width / 2)
                    grid_y += np.random.uniform(-cell_height / 2, cell_height / 2)
                    
                    self.init_pos[i] = grid_x
                    self.init_pos[placedb.num_nodes + i] = grid_y
            ...
            if len(placedb.regions) > 0:
                ...
                for i, region in enumerate(placedb.regions):
                    ...
                    for j, subregion in enumerate(region):
                        ...
                        self.init_pos[
                        placedb.num_physical_nodes + filler_beg + sub_filler_beg: 
                        placedb.num_physical_nodes + filler_beg + sub_filler_end] = np.random.uniform(
                            low=subregion[0],
                            high=subregion[2] - placedb.filler_size_x_fence_region[i],
                            size=sub_filler_end - sub_filler_beg,
                        )
                        self.init_pos[
                        placedb.num_nodes + placedb.num_physical_nodes + filler_beg + sub_filler_beg: 
                        placedb.num_nodes + placedb.num_physical_nodes + filler_beg + sub_filler_end] = np.random.uniform(
                            low=subregion[1],
                ...
                self.init_pos[
                placedb.num_physical_nodes + filler_beg: placedb.num_physical_nodes + filler_end] = np.random.uniform(
                    low=placedb.xl,
                    high=placedb.xh - placedb.filler_size_x_fence_region[-1],
                    size=filler_end - filler_beg,
                )
                self.init_pos[
                placedb.num_nodes + placedb.num_physical_nodes + filler_beg: placedb.num_nodes + placedb.num_physical_nodes + filler_end] = np.random.uniform(
                    low=placedb.yl,
                ...
                self.init_pos[
                placedb.num_nodes + placedb.num_physical_nodes: placedb.num_nodes * 2] = np.random.uniform(
                    low=placedb.yl,
        ...
        self.op_collections.hpwl_op = self.build_hpwl(params, placedb, 
                                                      self.data_collections, self.op_collections.pin_pos_op, self.device)
        ...
        if len(placedb.regions) > 0:
            self.op_collections.legalize_op, self.op_collections.individual_legalize_op = self.build_multi_fence_region_legalization(params, placedb, self.data_collections, self.device)
        else:
        ...
        if params.macro_place_flag:
            self.op_collections.macro_legalize_op = self.build_macro_legalization(params, placedb, self.data_collections, self.device)
    ...
    # Other methods are same as provided...
###move-layout-end###
        if placedb.num_filler_nodes:  # uniformly distribute filler cells in the layout
            if len(placedb.regions) > 0:
                ### uniformly spread fillers in fence region
    ...
    def build_multi_fence_region_legalization(self, params, placedb, data_collections, device):
        legal_ops = [self.build_fence_region_legalization(region_id, params, placedb, data_collections, device) for
                     region_id in range(len(placedb.regions) + 1)]
    ...
    def build_fence_region_legalization(self, region_id, params, placedb, data_collections, device):
        ...
        node_size_x = torch.cat([node_size_x[:num_movable_nodes][fence_region_mask],  ## movable
                                 ...
                                 node_size_x[
                                 num_movable_nodes + num_terminals:num_movable_nodes + num_terminals + num_terminal_NIs],
                                 ## terminal NIs
                                 node_size_x[
                                 num_nodes - num_filler_nodes + filler_beg:num_nodes - num_filler_nodes + filler_end]
                                 ## fillers
                                 ], 0)
        ...
        num_pins_in_nodes = torch.cat([num_pins_in_nodes[:num_movable_nodes][fence_region_mask],  ## movable
                                       num_pins_in_nodes[num_movable_nodes:num_movable_nodes + num_terminals],
                                       ## terminals
                                       torch.zeros(virtual_macros_size_x.size(0), dtype=num_pins_in_nodes.dtype,
                                                   device=device),  ## virtual macros
                                       num_pins_in_nodes[
                                       num_movable_nodes + num_terminals:num_movable_nodes + num_terminals + num_terminal_NIs],
                                       ## terminal NIs
                                       num_pins_in_nodes[
        ...
        assert node_size_x.size(0) == node_size_y.size(
            0) == num_movable_nodes_fence_region + num_terminals_fence_region + num_terminal_NIs + num_filler_nodes_fence_region
        ...
        flat_region_boxes_start = torch.tensor([0], device=node_size_x.device,
                                               dtype=data_collections.flat_region_boxes_start.dtype)
        ### node2fence region map: movable + terminal
        node2fence_region_map = torch.zeros(num_movable_nodes_fence_region + num_terminals_fence_region,
                                            dtype=data_collections.node2fence_region_map.dtype,
                                            device=node_size_x.device).fill_(
            data_collections.node2fence_region_map.max().item())
        ...
        def build_greedy_legalization_op(pos):
            ...
            pos_total[0, :num_movable_nodes].masked_scatter_(fence_region_mask,
                                                             result[0, :num_movable_nodes_fence_region])
            pos_total[1, :num_movable_nodes].masked_scatter_(fence_region_mask,
                                                             result[1, :num_movable_nodes_fence_region])
    ...
    def build_detailed_placement(self, params, placedb, data_collections,
        ...
        def build_detailed_placement_op(pos):
            ...
            if params.scale_factor != 1.0:
                ...
                logging.info(
                    "Use scale factor %g (1/%d) for detailed placement" % (scale_factor, target_inv_scale_factor))
