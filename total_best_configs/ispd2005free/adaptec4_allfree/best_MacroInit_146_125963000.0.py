...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Compute important statistics from placedb
        total_placeable_area = placedb.total_space_area - placedb.total_filler_node_area
        avg_node_area = placedb.total_movable_node_area / placedb.num_movable_nodes
        
        layout_center_x = (placedb.xl + placedb.xh) / 2
        layout_center_y = (placedb.yl + placedb.yh) / 2
        
        # x position
        self.init_pos[0:placedb.num_physical_nodes] = placedb.node_x
        
        # More refined initialization to spread macros based on density to minimize wire length
        if params.global_place_flag:
            cluster_radius_x = (placedb.xh - placedb.xl) / 4  # Smaller initial clusters
            cluster_radius_y = (placedb.yh - placedb.yl) / 4

            for i in range(placedb.num_movable_nodes):
                if placedb.node_size_x[i] * placedb.node_size_y[i] > avg_node_area:
                    # If the cell is a macro or larger cell, position it in a cluster around the center
                    self.init_pos[i] = np.random.normal(
                        loc=layout_center_x,
                        scale=cluster_radius_x / 2
                    )
                else:
                    # Otherwise, distribute more uniformly
                    self.init_pos[i] = np.random.uniform(
                        low=placedb.xl * 1.05, 
                        high=placedb.xh * 0.95
                    )
            ...
            for i in range(placedb.num_movable_nodes):
                if placedb.node_size_x[i] * placedb.node_size_y[i] > avg_node_area:
                    self.init_pos[placedb.num_nodes + i] = np.random.normal(
                        loc=layout_center_y,
                        scale=cluster_radius_y / 2
                    )
                else:
                    self.init_pos[placedb.num_nodes + i] = np.random.uniform(
                        low=placedb.yl * 1.05, 
                        high=placedb.yh * 0.95
                    )
