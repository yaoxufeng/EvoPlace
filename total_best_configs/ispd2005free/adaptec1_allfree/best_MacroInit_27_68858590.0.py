...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Calculate center positions and density-related factors
        macro_center_x = (placedb.xl + placedb.xh) / 2.0
        macro_center_y = (placedb.yl + placedb.yh) / 2.0
        cell_area_ratio = placedb.total_movable_cell_area / placedb.total_space_area
        macro_area_ratio = placedb.total_movable_macro_area / placedb.total_space_area
        total_space_ratio = placedb.total_movable_cell_area / (placedb.total_movable_cell_area + placedb.total_movable_macro_area)

        # Bias initial placement based on area ratios and center, with additional statistical insight
        cell_x_init_mean = macro_center_x * (1 - macro_area_ratio)
        cell_y_init_mean = macro_center_y * (1 - macro_area_ratio)
        
        macro_x_init_mean = macro_center_x * (1 + cell_area_ratio)
        macro_y_init_mean = macro_center_y * (1 + cell_area_ratio)

        # Compute standard deviation based on the ratio of movable macros and cells
        macro_scale = (placedb.xh - placedb.xl) * (0.003 + 0.002 * total_space_ratio)
        cell_scale = (placedb.xh - placedb.xl) * (0.003 * (1 - total_space_ratio))
        
        # Initial positions for macros with controlled spread
        self.init_pos[0:placedb.num_movable_nodes] = np.random.normal(
            loc=macro_x_init_mean,
            scale=macro_scale,
        ...
        self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_movable_nodes] = np.random.normal(
            loc=macro_y_init_mean,
            scale=macro_scale,
            size=placedb.num_movable_nodes
        )

        # Initial positions for cells
        self.init_pos[placedb.num_movable_nodes:placedb.num_physical_nodes] = np.random.normal(
            loc=cell_x_init_mean,
            scale=cell_scale,
            size=(placedb.num_physical_nodes - placedb.num_movable_nodes)
        )

        self.init_pos[placedb.num_nodes + placedb.num_movable_nodes:placedb.num_nodes + placedb.num_physical_nodes] = np.random.normal(
            loc=cell_y_init_mean,
            scale=cell_scale,
            size=(placedb.num_physical_nodes - placedb.num_movable_nodes)
        )
