...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        if params.global_place_flag and params.random_center_init_flag:
            ...
            cluster_size = int(np.sqrt(num_movable_nodes / (100))) + 1  # Number of clusters
            ...
            for cluster_id in range(cluster_size):
                ...
                cluster_center_x = np.random.uniform(placedb.xl, placedb.xh)
                cluster_center_y = np.random.uniform(placedb.yl, placedb.yh)

                self.init_pos[cluster_indices] = np.random.normal(
                    loc=cluster_center_x, 
                    scale=(placedb.xh - placedb.xl) / (10 * cluster_size),
                    size=len(cluster_indices)
                )
                self.init_pos[num_nodes + cluster_indices] = np.random.normal(
                    loc=cluster_center_y,
                    scale=(placedb.yh - placedb.yl) / (10 * cluster_size),
                    size=len(cluster_indices)
                )

        # Net-weight-based random noise
        net_weight_std_x = np.std(placedb.net_weights) * 0.001
        net_weight_std_y = np.std(placedb.net_weights) * 0.001
        ...
        # Iterative refinement loop using simulated annealing-like approach
        temperature = 1.0
        cooling_rate = 0.95
        for _ in range(10):  # Example iteration count for refinement
