...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        avg_node_size_x = np.mean(placedb.node_size_x)
        avg_node_size_y = np.mean(placedb.node_size_y)
        avg_bin_size_x = placedb.bin_size_x
        avg_bin_size_y = placedb.bin_size_y

        total_placeable_area = placedb.total_space_area
        num_bins_x = placedb.num_bins_x
        num_bins_y = placedb.num_bins_y

        # Calculate density-based center
        center_x = np.linspace(placedb.xl, placedb.xh, num_bins_x)
        center_y = np.linspace(placedb.yl, placedb.yh, num_bins_y)
        ...
        if params.global_place_flag:
            if params.random_center_init_flag:
                logging.info("Move cells to the center of layout with random noise based on density")
                density_center_x = avg_bin_size_x * (placedb.total_movable_cell_area / total_placeable_area)
                density_center_y = avg_bin_size_y * (placedb.total_movable_cell_area / total_placeable_area)

                self.init_pos[0:placedb.num_movable_nodes] = np.random.normal(
                    loc=density_center_x,
                    scale=avg_bin_size_x * 0.001,
                ...
                self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_movable_nodes] = np.random.normal(
                    loc=density_center_y,
                    scale=avg_bin_size_y * 0.001,
                    size=placedb.num_movable_nodes
                )
            else:
                logging.info("Move cells to weighted center of layout")
                weighted_x = (placedb.node_x * placedb.net_weights).sum() / placedb.net_weights.sum()
                weighted_y = (placedb.node_y * placedb.net_weights).sum() / placedb.net_weights.sum()

                self.init_pos[0:placedb.num_movable_nodes] = weighted_x
                self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_movable_nodes] = weighted_y
