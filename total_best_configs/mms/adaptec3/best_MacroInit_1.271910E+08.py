...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Calculate density scaling factor to balance the placement areas
        density_scaling = (placedb.total_space_area - placedb.total_filler_node_area) / placedb.total_movable_node_area

        # Use weighted average of center and initial position based on density scaling
        center_x = (placedb.xl + placedb.xh) / 2
        center_y = (placedb.yl + placedb.yh) / 2

        # X position initialization
        self.init_pos[0:placedb.num_physical_nodes] = (
            (1 - density_scaling) * placedb.node_x + density_scaling * center_x
        )
        
        # Random noise based on net weights to provide more dispersion
        ...
        self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_physical_nodes] = (
