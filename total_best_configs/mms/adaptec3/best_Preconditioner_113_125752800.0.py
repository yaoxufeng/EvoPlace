    ...
    def __call__(self, grad, density_weight, update_mask=None, fix_nodes_mask=None):
        ...
        with torch.no_grad():
            ...
            if density_weight.size(0) == 1:
                # Extract node sizes and compute areas
                node_widths = self.data_collections.node_size_x.float().to('cuda:0')
                node_heights = self.data_collections.node_size_y.float().to('cuda:0')
                node_areas = node_widths * node_heights
                
                # Extract statistical features from pin_offset arrays
                pin_offset_x = self.data_collections.pin_offset_x.float().to('cuda:0')
                pin_offset_y = self.data_collections.pin_offset_y.float().to('cuda:0')
                
                avg_pin_offset_x = pin_offset_x.mean()
                avg_pin_offset_y = pin_offset_y.mean()
                std_pin_offset_x = pin_offset_x.std()
                std_pin_offset_y = pin_offset_y.std()

                # Compute base preconditioner
                sum_pin_weights_in_nodes = self.op_collections.pws_op(self.data_collections.net_weights)
                base_precond = sum_pin_weights_in_nodes + self.alpha * density_weight * node_areas
                
                # Calculate adaptability based on region statistics
                region_factor = torch.exp(-0.5 * ((avg_pin_offset_x + avg_pin_offset_y) / (std_pin_offset_x + std_pin_offset_y + 1e-6)))
                
                # Apply adaptability to base preconditioner
                adaptive_preconditioner = base_precond * (1 + region_factor)
                
                # Return the final adaptive preconditioned gradient
                precond = adaptive_preconditioner
            ###pre-condition-end###
            else:
