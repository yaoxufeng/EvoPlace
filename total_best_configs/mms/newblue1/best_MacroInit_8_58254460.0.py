...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Determine x and y total available space
        x_center = (placedb.xl + placedb.xh) / 2
        y_center = (placedb.yl + placedb.yh) / 2
        
        x_noise = (placedb.xh - placedb.xl) * 0.001
        y_noise = (placedb.yh - placedb.yl) * 0.001
        
        # Compute density utilization
        used_space_ratio = placedb.total_movable_node_area / placedb.total_space_area
        macro_scaling_factor = 1.0 / (1.0 + used_space_ratio)
        
        # Initial x position
        self.init_pos[0:placedb.num_physical_nodes] = placedb.node_x
        if params.global_place_flag and params.random_center_init_flag:
            logging.info("Use density-aware macro initialization with noise")
            self.init_pos[0:placedb.num_movable_nodes] = np.random.normal(
                loc=x_center * macro_scaling_factor, 
                scale=x_noise, 
        ...
        if params.global_place_flag and params.random_center_init_flag:
            self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_movable_nodes] = np.random.normal(
                loc=y_center * macro_scaling_factor, 
                scale=y_noise, 
                size=placedb.num_movable_nodes)
###move-layout-end###
