...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Apply clustering-based initialization if random_center_init_flag is set
        if params.global_place_flag and params.random_center_init_flag:
            logging.info("Applying clustering-based initialization for movable nodes")
            movable_cells = np.random.permutation(placedb.num_movable_nodes)
            cluster_centers_x = np.linspace(placedb.xl, placedb.xh, num=10)
            cluster_centers_y = np.linspace(placedb.yl, placedb.yh, num=10)

            for i in range(placedb.num_movable_nodes):
                cluster_idx = movable_cells[i] % len(cluster_centers_x)
                self.init_pos[movable_cells[i]] = np.random.normal(
                    loc=cluster_centers_x[cluster_idx],
                    scale=(placedb.xh - placedb.xl) * 0.01
                )
                self.init_pos[placedb.num_nodes + movable_cells[i]] = np.random.normal(
                    loc=cluster_centers_y[cluster_idx],
                    scale=(placedb.yh - placedb.yl) * 0.01
                )
        ...
        if params.global_place_flag and params.random_center_init_flag:
            for i in range(placedb.num_movable_nodes):
                cluster_idx = i % len(cluster_centers_y)
                self.init_pos[placedb.num_nodes + i] = np.random.normal(
                    loc=cluster_centers_y[cluster_idx],
                    scale=(placedb.yh - placedb.yl) * 0.01
                )

        # Density-aware placement
        total_area = placedb.total_space_area
        movable_area = placedb.total_movable_node_area
        density = movable_area / total_area

        if density > 0.9:  # Arbitrary threshold, adjust based on experience
            logging.info("High density detected, spreading cells across more evenly...")
            num_bins_x, num_bins_y = placedb.num_bins_x, placedb.num_bins_y
            bin_grid_x = np.linspace(placedb.xl, placedb.xh, num=num_bins_x)
            bin_grid_y = np.linspace(placedb.yl, placedb.yh, num=num_bins_y)
            
            for i in range(placedb.num_movable_nodes):
                bin_x = i % num_bins_x
                bin_y = (i // num_bins_x) % num_bins_y
                self.init_pos[i] = np.random.normal(
                    loc=bin_grid_x[bin_x],
                    scale=placedb.bin_size_x * 0.01
                )
                self.init_pos[placedb.num_nodes + i] = np.random.normal(
                    loc=bin_grid_y[bin_y],
                    scale=placedb.bin_size_y * 0.01
                )

        # Proximity to Fixed Blocks
        placed_block_indices = [i for i in range(placedb.num_physical_nodes) if i >= placedb.num_movable_nodes]
        if placed_block_indices:
            fixed_block_x = placedb.node_x[placed_block_indices]
            fixed_block_y = placedb.node_y[placed_block_indices]
            for i in range(placedb.num_movable_nodes):
                closest_block_idx = i % len(fixed_block_x)
                self.init_pos[i] = fixed_block_x[closest_block_idx] + np.random.uniform(0, placedb.site_width)
                self.init_pos[placedb.num_nodes + i] = fixed_block_y[closest_block_idx] + np.random.uniform(0, placedb.row_height)
###move-layout-end###
