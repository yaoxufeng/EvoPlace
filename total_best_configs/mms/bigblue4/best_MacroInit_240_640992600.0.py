...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Calculate density and a more informed scale
        node_density = placedb.total_movable_node_area / placedb.total_space_area
        scale_x = layout_width * 0.001 * np.sqrt(node_density)
        scale_y = layout_height * 0.001 * np.sqrt(node_density)
        ...
        if params.global_place_flag and params.random_center_init_flag:
            logging.info("Improved macro placement with density-aware scaling")
