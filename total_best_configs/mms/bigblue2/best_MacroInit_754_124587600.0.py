...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        if params.global_place_flag and params.random_center_init_flag:
            logging.info("Using improved initialization strategy for movable nodes")
            
            # Determine central region with reduced random noise for movable nodes
            center_x = (placedb.xl + placedb.xh) / 2
            center_y = (placedb.yl + placedb.yh) / 2
            
            # Move macros to the center with strategic spreading based on macro stats
            node_sizes_x = placedb.node_size_x[:placedb.num_movable_nodes]
            node_sizes_y = placedb.node_size_y[:placedb.num_movable_nodes]

            macros_indices = np.where(node_sizes_x * node_sizes_y > placedb.row_height * placedb.site_width * 10)[0]
            cells_indices = np.setdiff1d(np.arange(placedb.num_movable_nodes), macros_indices)

            # For macros: spread them around the center with slight noise
            self.init_pos[macros_indices] = np.random.normal(
                loc=center_x, scale=(placedb.xh - placedb.xl) * 0.002, size=(len(macros_indices))
            )
            self.init_pos[placedb.num_nodes + macros_indices] = np.random.normal(
                loc=center_y, scale=(placedb.yh - placedb.yl) * 0.002, size=(len(macros_indices))
            )
            
            # For standard cells: use a broader spread around the center to distribute load
            self.init_pos[cells_indices] = np.random.normal(
                loc=center_x, scale=(placedb.xh - placedb.xl) * 0.01, size=(len(cells_indices))
            )
            self.init_pos[placedb.num_nodes + cells_indices] = np.random.normal(
                loc=center_y, scale=(placedb.yh - placedb.yl) * 0.01, size=(len(cells_indices))
            )
###move-layout-end###
