    ...
    def __call__(self, grad, density_weight, update_mask=None, fix_nodes_mask=None):
        ...
        with torch.no_grad():
            ...
            if density_weight.size(0) == 1:
                # Calculate statistical information for pin offsets
                pin_offset_x_mean = torch.mean(self.data_collections.pin_offset_x)
                pin_offset_y_mean = torch.mean(self.data_collections.pin_offset_y)
                pin_offset_x_std = torch.std(self.data_collections.pin_offset_x)
                pin_offset_y_std = torch.std(self.data_collections.pin_offset_y)

                # Compute density levels for adaptive preconditioning
                area_mean = torch.mean(self.data_collections.node_areas)
                area_std = torch.std(self.data_collections.node_areas)
                density_levels = (self.data_collections.node_areas - area_mean) / area_std

                # Classify regions based on density
                high_density_region = (density_levels > 1.0).float().to(density_weight.device)
                moderate_density_region = ((density_levels <= 1.0) & (density_levels >= -1.0)).float().to(density_weight.device)
                low_density_region = (density_levels < -1.0).float().to(density_weight.device)

                # Calculate pin weights impact and sensitivity
                node_pin_weight_impact = self.data_collections.num_pins_in_nodes.float() / torch.max(self.data_collections.num_pins_in_nodes)
                pin_sensitivity_x = (self.data_collections.pin_offset_x - pin_offset_x_mean) / pin_offset_x_std
                pin_sensitivity_y = (self.data_collections.pin_offset_y - pin_offset_y_mean) / pin_offset_y_std
                pin_sensitivity = torch.sqrt(pin_sensitivity_x**2 + pin_sensitivity_y**2)
                
                # Aggregate influence factors
                influence_factors = high_density_region*1.5 + moderate_density_region + low_density_region*0.5
                influence_factors = influence_factors * node_pin_weight_impact[:influence_factors.size(0)].to(density_weight.device)

                # Preconditioner using adjusted alpha
                adjusted_alpha = self.alpha * (1 + pin_sensitivity[:influence_factors.size(0)].to(density_weight.device))

                # Final preconditioner calculation
                precond = (self.op_collections.pws_op(self.data_collections.net_weights)
                           + adjusted_alpha * density_weight * influence_factors * self.data_collections.node_areas)
            ###pre-condition-end###
            else:
