...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Calculate inter-quartile regions
        interquartile_x = np.percentile(placedb.node_x, [25, 75])
        interquartile_y = np.percentile(placedb.node_y, [25, 75])

        # Use a strategy to initialize positions based on congestion and density
        if params.global_place_flag:
            logging.info("Improved initialization with congestion and density awareness")

            # Initialize movable nodes position based on inter-quartile region
            for i in range(placedb.num_movable_nodes):
                node_width = placedb.node_size_x[i]
                node_height = placedb.node_size_y[i]
                
                # Adjust placement towards lower congestion areas
                x_init = np.random.normal(
                    loc=(interquartile_x[0] + interquartile_x[1]) / 2, 
                    scale=(interquartile_x[1] - interquartile_x[0]) * 0.1
                )
                y_init = np.random.normal(
                    loc=(interquartile_y[0] + interquartile_y[1]) / 2, 
                    scale=(interquartile_y[1] - interquartile_y[0]) * 0.1
                )
                
                # Consider density and movable area
                density_factor_x = placedb.total_movable_macro_area / placedb.total_space_area
                density_factor_y = placedb.total_movable_cell_area / placedb.total_space_area
                
                # Modulate position with density awareness
                x_init = center_x + (x_init - center_x) * density_factor_x
                y_init = center_y + (y_init - center_y) * density_factor_y
                
                self.init_pos[i] = x_init
                self.init_pos[placedb.num_nodes + i] = y_init
        
        # Initialize fixed nodes as per original positions
        for i in range(placedb.num_movable_nodes, placedb.num_physical_nodes):
            self.init_pos[i] = placedb.node_x[i]
            self.init_pos[placedb.num_nodes + i] = placedb.node_y[i]
###move-layout-end###
