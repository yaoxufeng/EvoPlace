...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        placement_density = placedb.total_movable_node_area / placedb.total_space_area
        density_scaling_factor = max(1.0, 1.2 - placement_density)

        # Calculate layout center
        center_x = (placedb.xl + placedb.xh) / 2
        center_y = (placedb.yl + placedb.yh) / 2

        # Adaptively scale x positions
        self.init_pos[0:num_physical_nodes] = (
            (1 - density_scaling_factor) * placedb.node_x + density_scaling_factor * center_x
        )

        # Adaptively scale y positions
        self.init_pos[num_nodes:num_nodes + num_physical_nodes] = (
            (1 - density_scaling_factor) * placedb.node_y + density_scaling_factor * center_y
        )

        # Advanced Cluster-based Initialization
        if params.global_place_flag and params.random_center_init_flag:
            cluster_factor = int(np.sqrt(num_movable_nodes / 150)) + 1
            node_clusters = np.random.randint(0, cluster_factor, num_movable_nodes)

            for cluster_id in range(cluster_factor):
                idx = np.where(node_clusters == cluster_id)[0]
                cluster_center_x = np.random.uniform(placedb.xl, placedb.xh)
                cluster_center_y = np.random.uniform(placedb.yl, placedb.yh)

                self.init_pos[idx] = np.random.normal(
                    loc=cluster_center_x,
                    scale=(placedb.xh - placedb.xl) / (15 * cluster_factor),
                    size=len(idx)
                )
                self.init_pos[num_nodes + idx] = np.random.normal(
                    loc=cluster_center_y,
                    scale=(placedb.yh - placedb.yl) / (15 * cluster_factor),
                    size=len(idx)
                )

        # Net-weight-informed noise addition
        net_weight_std_x = np.std(placedb.net_weights) * 0.002
        net_weight_std_y = np.std(placedb.net_weights) * 0.002
        ...
        # Iterative Refinement with Early Cooling
        initial_temperature = 1.0
        cooling_rate = 0.90
        refinement_iterations = 7

        for _ in range(refinement_iterations):
            perturb_x = np.random.uniform(-1, 1, size=num_movable_nodes) * initial_temperature
            perturb_y = np.random.uniform(-1, 1, size=num_movable_nodes) * initial_temperature
