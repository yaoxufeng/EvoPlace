...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Define layout center
        center_x = (placedb.xl + placedb.xh) / 2
        center_y = (placedb.yl + placedb.yh) / 2

        # Gather statistical information
        avg_pin_offset_x = np.mean(placedb.pin_offset_x[:placedb.num_movable_pins])
        avg_pin_offset_y = np.mean(placedb.pin_offset_y[:placedb.num_movable_pins])
        avg_node_size_x = np.mean(placedb.node_size_x[:placedb.num_movable_nodes])
        avg_node_size_y = np.mean(placedb.node_size_y[:placedb.num_movable_nodes])

        # Positioning macros and cells with improved strategy
        # X position
        self.init_pos[0:placedb.num_physical_nodes] = placedb.node_x

        if params.global_place_flag:
            logging.info("Positioning movable cells using improved strategy")

            # Spread cells around the center with constraints
            max_spread = min(placedb.bin_size_x, placedb.bin_size_y)
            
            # Init positions with a Gaussian spread relative to the average node size
            self.init_pos[0:placedb.num_movable_nodes] = np.random.normal(
                loc=center_x + avg_pin_offset_x,
                scale=max_spread / avg_node_size_x,
        ...
        if params.global_place_flag:
            ...
            self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_movable_nodes] = np.random.normal(
                loc=center_y + avg_pin_offset_y,
                scale=max_spread / avg_node_size_y,
                size=placedb.num_movable_nodes
            )
###move-layout-end###
