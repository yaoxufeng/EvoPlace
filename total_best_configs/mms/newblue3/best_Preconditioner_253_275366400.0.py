    ...
    def __call__(self, grad, density_weight, update_mask=None, fix_nodes_mask=None):
        ...
        with torch.no_grad():
            ...
            if density_weight.size(0) == 1:
                device = self.data_collections.node_size_x.device
                ...
                num_pins_in_nodes = self.data_collections.num_pins_in_nodes
                sum_pin_weights_in_nodes = sum_pin_weights_in_nodes.to(device)

                # Extract statistical features
                pin_offset_x_mean = torch.mean(self.data_collections.pin_offset_x.to(device))
                pin_offset_x_var = torch.var(self.data_collections.pin_offset_x.to(device))
                pin_offset_y_mean = torch.mean(self.data_collections.pin_offset_y.to(device))
                pin_offset_y_var = torch.var(self.data_collections.pin_offset_y.to(device))

                # Additional statistics
                size_x_mean = torch.mean(node_size_x)
                size_x_var = torch.var(node_size_x)
                size_y_mean = torch.mean(node_size_y)
                size_y_var = torch.var(node_size_y)
                pin_counts_mean = torch.mean(num_pins_in_nodes)
                pin_counts_var = torch.var(num_pins_in_nodes)

                # Combine features for wirelength prediction
                features = torch.stack([
                    size_x_mean, size_x_var, size_y_mean, size_y_var,
                    pin_counts_mean, pin_counts_var,
                    pin_offset_x_mean, pin_offset_x_var,
                    pin_offset_y_mean, pin_offset_y_var
                ], dim=0).to(device)

                # Model parameters (to simulate a trained model)
                model_weights = torch.randn((features.size(0),), device=device) * 0.1
                model_bias = torch.tensor(0.1, device=device)

                # Wirelength adjustment prediction (simple linear model for demonstration)
                wirelength_adjustment = torch.dot(model_weights, features) + model_bias

                # Ensure no division by zero and scale densities properly
                density_weight_adjusted = torch.clamp(density_weight, min=1e-3) * (1 + torch.sigmoid(wirelength_adjustment))

                # Define the preconditioner
                precond = (
                    sum_pin_weights_in_nodes +
                    self.alpha * density_weight_adjusted * node_areas
                )
            ###pre-condition-end###
            else:
