...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        self.init_pos = np.zeros(num_nodes * 2, dtype=dtype)

        # Calculate density scaling factor
        density_scaling = min(1.0, (placedb.total_space_area - placedb.total_filler_node_area) / placedb.total_movable_node_area)

        # Layout center
        center_x = (placedb.xl + placedb.xh) / 2
        center_y = (placedb.yl + placedb.yh) / 2

        # Use median instead of uniform center to prevent large initial wirelengths
        node_x_median = np.median(placedb.node_x[:num_movable_nodes])
        node_y_median = np.median(placedb.node_y[:num_movable_nodes])

        # X position initialization
        self.init_pos[0:num_physical_nodes] = (
            (1 - density_scaling) * node_x_median + density_scaling * center_x
        )

        # Y position initialization
        self.init_pos[num_nodes:num_nodes + num_physical_nodes] = (
            (1 - density_scaling) * node_y_median + density_scaling * center_y
        )

        if params.global_place_flag:
            # Initialize positions with a cluster-based approach
            cluster_size = int(np.sqrt(num_movable_nodes / 50)) + 1  # Adjusted cluster size
            node_clusters = np.random.randint(0, cluster_size, num_movable_nodes)

            # Implement hierarchical clustering improvement
            for cluster_id in range(cluster_size):
                cluster_indices = np.where(node_clusters == cluster_id)[0]

                # Hierarchical centers based on net connectivity density
                centroid_x = np.mean(placedb.node_x[cluster_indices])
                centroid_y = np.mean(placedb.node_y[cluster_indices])
                spread_x = (placedb.xh - placedb.xl) / cluster_size
                spread_y = (placedb.yh - placedb.yl) / cluster_size

                self.init_pos[cluster_indices] = np.random.normal(
                    loc=centroid_x,
                    scale=spread_x,
                    size=len(cluster_indices)
                ).clip(placedb.xl, placedb.xh)

                self.init_pos[num_nodes + cluster_indices] = np.random.normal(
                    loc=centroid_y,
                    scale=spread_y,
                    size=len(cluster_indices)
                ).clip(placedb.yl, placedb.yh)

            # Cluster-based perturbation using net weights
            net_weight_std_x = max(1.0, np.std(placedb.net_weights) * 0.001)
            net_weight_std_y = max(1.0, np.std(placedb.net_weights) * 0.001)

            self.init_pos[0:num_movable_nodes] += np.random.normal(
                loc=0,
                scale=net_weight_std_x,
                size=num_movable_nodes
            )
            self.init_pos[num_nodes:num_nodes + num_movable_nodes] += np.random.normal(
                loc=0,
                scale=net_weight_std_y,
                size=num_movable_nodes
            )

            # Tighter refinement loop with adjusted cooling schedule
            temperature = 10.0  # Adjusted for large initial perturbation
            cooling_rate = 0.9  # Faster cooling for quicker stabilization
            for _ in range(15):  # Increase iterations for more refinement
                perturb_x = np.random.uniform(-1, 1, size=num_movable_nodes) * temperature
                perturb_y = np.random.uniform(-1, 1, size=num_movable_nodes) * temperature

                self.init_pos[0:num_movable_nodes] += perturb_x
                self.init_pos[num_nodes:num_nodes + num_movable_nodes] += perturb_y
                temperature *= cooling_rate

        # Uniform distribution of filler cells
        if placedb.num_filler_nodes:
            self.init_pos[placedb.num_physical_nodes:placedb.num_nodes] = np.random.uniform(
                low=placedb.xl + placedb.site_width,
                high=placedb.xh - placedb.site_width,
                size=placedb.num_filler_nodes
            )
            self.init_pos[placedb.num_nodes + placedb.num_physical_nodes: placedb.num_nodes * 2] = np.random.uniform(
                low=placedb.yl + placedb.row_height,
                high=placedb.yh - placedb.row_height,
                size=placedb.num_filler_nodes
            )
