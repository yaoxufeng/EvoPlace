...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Compute center position for macros and standard cells
        x_center = (placedb.xl + placedb.xh) / 2
        y_center = (placedb.yl + placedb.yh) / 2

        # Determine macro positions
        for i in range(placedb.num_physical_nodes):
            macro_width_ratio = placedb.node_size_x[i] / placedb.total_movable_node_area
            macro_height_ratio = placedb.node_size_y[i] / placedb.total_movable_node_area
            
            self.init_pos[i] = (1 - macro_width_ratio) * x_center + macro_width_ratio * np.random.uniform(placedb.xl, placedb.xh)
            self.init_pos[placedb.num_nodes + i] = (1 - macro_height_ratio) * y_center + macro_height_ratio * np.random.uniform(placedb.yl, placedb.yh)

        # Apply random noise for initial placement of movable cells
        if params.global_place_flag and params.random_center_init_flag:
            std_dev_x = (placedb.xh - placedb.xl) * 0.001
            std_dev_y = (placedb.yh - placedb.yl) * 0.001
            
            self.init_pos[0:placedb.num_movable_nodes] += np.random.normal(0, std_dev_x, size=placedb.num_movable_nodes)
            self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_movable_nodes] += np.random.normal(0, std_dev_y, size=placedb.num_movable_nodes)
###move-layout-end###
