...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Determine the center of the layout area
        center_x = (placedb.xl + placedb.xh) / 2
        center_y = (placedb.yl + placedb.yh) / 2
        
        # Calculate scaling factors for more informed spreading
        density_factor = placedb.total_movable_node_area / placedb.total_space_area
        noise_scale_x = (placedb.xh - placedb.xl) * 0.01 * density_factor
        noise_scale_y = (placedb.yh - placedb.yl) * 0.01 * density_factor
        
        # Calculate initial positions with improved strategy
        ...
        if params.global_place_flag:
            # Move cells to the strategic positions near the initial center and spread with controlled randomness
            target_x = np.random.normal(
                loc=center_x,
                scale=noise_scale_x,
                size=placedb.num_movable_nodes
            )
            target_y = np.random.normal(
                loc=center_y,
                scale=noise_scale_y,
                size=placedb.num_movable_nodes
            )
            
            # Adjust positions to ensure minimized initial HPWL and better convergence
            self.init_pos[0:placedb.num_movable_nodes] = placedb.node_x[0:placedb.num_movable_nodes] + target_x
            self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_movable_nodes] = placedb.node_y[0:placedb.num_movable_nodes] + target_y
###move-layout-end###
