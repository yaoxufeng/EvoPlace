...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Function to cluster cells and their initial positions
        def cluster_cells(node_x, node_y, cluster_size):
            clusters = {}
            for i in range(0, len(node_x), cluster_size):
                cluster_indices = range(i, min(i + cluster_size, len(node_x)))
                center_x = np.mean([node_x[idx] for idx in cluster_indices])
                center_y = np.mean([node_y[idx] for idx in cluster_indices])
                clusters[i // cluster_size] = (center_x, center_y, cluster_indices)
            return clusters

        # Clustering cells based on their initial positions
        cluster_size = max(1, int(len(placedb.node_x) ** 0.5))  # heuristic cluster size
        clusters = cluster_cells(placedb.node_x, placedb.node_y, cluster_size)

        # Assign clusters to prevent initial overlap and better distribution
        for cluster_id, (center_x, center_y, cluster_indices) in clusters.items():
            for idx in cluster_indices:
                self.init_pos[idx] = center_x + np.random.uniform(-placedb.bin_size_x, placedb.bin_size_x) * 0.1
                self.init_pos[idx + placedb.num_nodes] = center_y + np.random.uniform(-placedb.bin_size_y, placedb.bin_size_y) * 0.1

        # Move to the center of layout with controlled noise
        if params.global_place_flag and params.random_center_init_flag:
            logging.info("Adjusting initial macro positions")
            spread_factor = 0.001 / max(cluster_size, 1)
            for idx in range(placedb.num_movable_nodes):
                self.init_pos[idx] = np.random.normal(
                    loc=(placedb.xl + placedb.xh) / 2,
                    scale=(placedb.xh - placedb.xl) * spread_factor
                )
                self.init_pos[placedb.num_nodes + idx] = np.random.normal(
                    loc=(placedb.yl + placedb.yh) / 2,
                    scale=(placedb.yh - placedb.yl) * spread_factor
                )

        logging.info(f"Initialization done in {time.time() - tt} seconds.")
###move-layout-end###
