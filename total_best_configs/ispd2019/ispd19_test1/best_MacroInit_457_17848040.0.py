...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        if params.global_place_flag and params.random_center_init_flag:
            ...
            # Use macros' statistical center as an anchor to position movable cells
            macro_center_x = (placedb.total_movable_macro_area / 
                              placedb.total_movable_node_area) * (
                (placedb.xl + placedb.xh) / 2.0)
            macro_center_x_noisy = np.random.normal(
                loc=macro_center_x,
                scale=(placedb.xh - placedb.xl) * 0.001,
                size=placedb.num_movable_nodes
            )

            self.init_pos[:placedb.num_movable_nodes] = macro_center_x_noisy
        ...
        if params.global_place_flag and params.random_center_init_flag:
            # Use macros' statistical center as an anchor to position movable cells
            macro_center_y = (placedb.total_movable_macro_area / 
                              placedb.total_movable_node_area) * (
                (placedb.yl + placedb.yh) / 2.0)
            macro_center_y_noisy = np.random.normal(
                loc=macro_center_y,
            ...
            self.init_pos[placedb.num_nodes:
                          placedb.num_nodes + placedb.num_movable_nodes] = macro_center_y_noisy
###move-layout-end###
