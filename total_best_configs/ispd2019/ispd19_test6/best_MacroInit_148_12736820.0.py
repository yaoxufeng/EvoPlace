...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Center coordinates of the layout
        center_x = (placedb.xl + placedb.xh) / 2
        center_y = (placedb.yl + placedb.yh) / 2

        # Calculate density factors based on node sizes
        density_factor_x = placedb.total_movable_node_area / placedb.total_space_area
        density_factor_y = placedb.total_movable_node_area / placedb.total_space_area

        # Initial Placement for x and y based on density and clustering
        self.init_pos[0:placedb.num_physical_nodes] = center_x + (placedb.node_size_x[:placedb.num_physical_nodes] * density_factor_x / 2)
        self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_physical_nodes] = center_y + (placedb.node_size_y[:placedb.num_physical_nodes] * density_factor_y / 2)
        
        if params.global_place_flag and params.random_center_init_flag: 
            # Apply random perturbation around the center
            perturbation_scale_x = (placedb.xh - placedb.xl) * 0.01
            perturbation_scale_y = (placedb.yh - placedb.yl) * 0.01

            self.init_pos[0:placedb.num_movable_nodes] += np.random.normal(scale=perturbation_scale_x, size=placedb.num_movable_nodes)
            self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_movable_nodes] += np.random.normal(scale=perturbation_scale_y, size=placedb.num_movable_nodes)
###move-layout-end###
