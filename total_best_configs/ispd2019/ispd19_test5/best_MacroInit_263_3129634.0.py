...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Calculate connectivity-based centroids
        connectivity_center_x = np.average(placedb.node_x[:placedb.num_movable_nodes], weights=placedb.net_weights[:placedb.num_movable_nodes])
        connectivity_center_y = np.average(placedb.node_y[:placedb.num_movable_nodes], weights=placedb.net_weights[:placedb.num_movable_nodes])
        
        # Clustering based initial placement
        # Initialize clusters for movable nodes
        cluster_std_x = (placedb.xh - placedb.xl) * 0.005  # Smaller variance for tighter placement
        cluster_std_y = (placedb.yh - placedb.yl) * 0.005
        
        # Adjust positions based on clustering centroids
        self.init_pos[:placedb.num_movable_nodes] = np.random.normal(
            loc=connectivity_center_x, 
            scale=cluster_std_x, 
        ...
        self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_movable_nodes] = np.random.normal(
            loc=connectivity_center_y, 
            scale=cluster_std_y, 
            size=placedb.num_movable_nodes
        )
        
        # Consider aspect ratios while placing non-movable physical nodes
        for i in range(placedb.num_movable_nodes, placedb.num_physical_nodes):
            if placedb.node_size_x[i] > placedb.node_size_y[i]:
                # Prefer horizontal placement
                self.init_pos[i] = placedb.node_x[i] + np.random.uniform(-placedb.site_width, placedb.site_width)
                self.init_pos[placedb.num_nodes + i] = placedb.node_y[i]
            else:
                # Prefer vertical placement
                self.init_pos[i] = placedb.node_x[i]
                self.init_pos[placedb.num_nodes + i] = placedb.node_y[i] + np.random.uniform(-placedb.row_height, placedb.row_height)
###move-layout-end###
