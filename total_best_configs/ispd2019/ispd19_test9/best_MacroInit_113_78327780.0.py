...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Improved Density Scaling
        density_scaling = max(1.0, (placedb.total_space_area - placedb.total_filler_node_area) / placedb.total_movable_node_area)
        ...
        self.init_pos[0:num_physical_nodes] = (
            (1 - density_scaling) * placedb.node_x + density_scaling * center_x
        )

        # Y position initialization
        self.init_pos[num_nodes:num_nodes + num_physical_nodes] = (
            (1 - density_scaling) * placedb.node_y + density_scaling * center_y
        )

        if params.global_place_flag and params.random_center_init_flag:
            # Enhanced Clustering Initialization
            cluster_ratio = 0.05
            cluster_size = max(1, int(num_movable_nodes * cluster_ratio))
            node_clusters = np.random.choice(range(cluster_size), num_movable_nodes)

            for cluster_id in range(cluster_size):
                cluster_indices = np.where(node_clusters == cluster_id)[0]
                cluster_center_x = np.random.uniform(center_x - 10, center_x + 10)
                cluster_center_y = np.random.uniform(center_y - 10, center_y + 10)

                self.init_pos[cluster_indices] = np.random.normal(
                    loc=cluster_center_x,
                    scale=(placedb.xh - placedb.xl) / (20 * cluster_size),
                    size=len(cluster_indices)
                )
                self.init_pos[num_nodes + cluster_indices] = np.random.normal(
                    loc=cluster_center_y,
                    scale=(placedb.yh - placedb.yl) / (20 * cluster_size),
                    size=len(cluster_indices)
                )

        # Strategic Net-Weight-Based Random Noise
        net_weight_std_x = np.std(placedb.net_weights) * 0.0005
        net_weight_std_y = np.std(placedb.net_weights) * 0.0005
        ...
        temperature = 1.0
        cooling_rate = 0.90
        for _ in range(5):  # Reduced iteration for larger initial moves
        ...
        # Filler Initialization as Uniform Distribution
        if placedb.num_filler_nodes:
            self.init_pos[num_physical_nodes: num_nodes] = np.random.uniform(
                low=placedb.xl,
                high=placedb.xh - placedb.node_size_x[-placedb.num_filler_nodes],
                size=placedb.num_filler_nodes,
            )
            self.init_pos[num_nodes + num_physical_nodes: num_nodes * 2] = np.random.uniform(
                low=placedb.yl,
