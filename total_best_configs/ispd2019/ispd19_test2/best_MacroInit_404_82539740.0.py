...
class BasicPlace(nn.Module):
    def __init__(self, params, placedb, timer):
        ...
        # Initial x position (Move to center with consideration of macro and cell areas)
        self.init_pos[0:placedb.num_physical_nodes] = placedb.node_x
        
        if params.global_place_flag and params.random_center_init_flag:
            logging.info("Move cells to the center of layout with controlled random noise")
            
            # For macros (large movable nodes), spread them based on available space to avoid overlaps
            total_area = placedb.total_space_area - placedb.total_filler_node_area
            macro_area_fraction = placedb.total_movable_macro_area / total_area
            cell_area_fraction = placedb.total_movable_cell_area / total_area
            
            macro_dispersion_factor = 0.5  # A greater factor spreads macros more evenly
            cell_dispersion_factor = 0.1   # Cells require less dispersion

            # Spreading macros and cells based on calculated dispersion factors
            macro_x_location = (placedb.xl + placedb.xh) * 0.5
            macro_x_scale = (placedb.xh - placedb.xl) * 0.5 * macro_dispersion_factor * np.sqrt(macro_area_fraction)
            
            cell_x_location = (placedb.xl + placedb.xh) * 0.5
            cell_x_scale = (placedb.xh - placedb.xl) * 0.5 * cell_dispersion_factor * np.sqrt(cell_area_fraction)
            
            macro_indices = np.arange(placedb.num_nodes)[placedb.num_nets:]
            cell_indices = np.arange(placedb.num_movable_nodes)
            
            self.init_pos[macro_indices] = np.random.normal(
                loc=macro_x_location,
                scale=macro_x_scale,
                size=len(macro_indices)
            )
            self.init_pos[cell_indices] = np.random.normal(
                loc=cell_x_location,
                scale=cell_x_scale,
                size=len(cell_indices)
            )

        # Initial y position (similar strategy as x position)
        self.init_pos[placedb.num_nodes:placedb.num_nodes + placedb.num_physical_nodes] = placedb.node_y
        
        if params.global_place_flag and params.random_center_init_flag:
            macro_y_location = (placedb.yl + placedb.yh) * 0.5
            macro_y_scale = (placedb.yh - placedb.yl) * 0.5 * macro_dispersion_factor * np.sqrt(macro_area_fraction)
            
            cell_y_location = (placedb.yl + placedb.yh) * 0.5
            cell_y_scale = (placedb.yh - placedb.yl) * 0.5 * cell_dispersion_factor * np.sqrt(cell_area_fraction)
            
            self.init_pos[placedb.num_nodes + macro_indices] = np.random.normal(
                loc=macro_y_location,
                scale=macro_y_scale,
                size=len(macro_indices)
            )
            self.init_pos[placedb.num_nodes + cell_indices] = np.random.normal(
                loc=cell_y_location,
                scale=cell_y_scale,
                size=len(cell_indices)
            )
